...
module.exports = async (client, member) => {
  ...
  if(config.leaveType == 'embed') {
    ...
    .setAuthor(`We want you in ${member.guild.name} :(`, member.guild.iconURL({dynamic: true}))
    .setDescription(msg)
    .setColor('BLUE')
    .setTimestamp();
  ...
  } else if(config.leaveType == 'image') {
    ...
    text = `Now we're ${member.guild.memberCount} members!`,
    number = `- ${member.memberCount}th member !`,
    title = 'GoodBye';
    ...
    let attachment = new MessageAttachment(canvas.toBuffer(), "goodbye-image.png");
    client.channels.cache.get('785286660748869683').send(attachment);
  }
}